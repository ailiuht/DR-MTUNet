...
class DR_MTUNet(nn.Module):
    def __init__(self, in_ch=3, seg_ch=2, recon_ch=3, shape_dim=5):
        ...
        # 动态残差门控
        self.gate1 = DynamicResidualGate(64, shape_dim)
        self.gate2 = DynamicResidualGate(128, shape_dim)
        self.gate3 = DynamicResidualGate(256, shape_dim)
        self.gate4 = DynamicResidualGate(512, shape_dim)
        # 形状参数预测
        ...
        # # 形状参数预测
        self.ellipse_predictor = EllipseParamPredictor(in_chs=seg_ch)
    ...
    def forward(self, x):
        ...
        shape_params = self.ellipse_predictor(segs)

        # 动态门控跳跃连接
        d4 = torch.cat([self.gate4(e4, shape_params), bottleneck_out], dim=1)
        d4 = self.dec4(d4)

        d3 = self.up3(d4)
        d3 = torch.cat([self.gate3(e3, shape_params), d3], dim=1)
        d3 = self.dec3(d3)

        d2 = self.up2(d3)
        d2 = torch.cat([self.gate2(e2, shape_params), d2], dim=1)
        d2 = self.dec2(d2)

        d1 = self.up1(d2)
        d1 = torch.cat([self.gate1(e1, shape_params), d1], dim=1)
        d1 = self.dec1(d1)
        ...
        return combined_seg, recon, shape_params, alpha
